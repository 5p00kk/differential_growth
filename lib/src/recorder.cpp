...
bool c_recorder::new_recording(unsigned int width, unsigned int height, const std::string &path)
{
    m_path = path;
    m_frame_number = 0;
    m_recording = m_video_writer.open((path+"dg.mp4"), cv::VideoWriter::fourcc('a', 'v', 'c', '1'), 60, cv::Size(height, width), false);
    return m_recording;
}
...
void c_recorder::save_image(const cv::Mat &frame)
{
    if(m_recording)
    {
        std::string image_path = m_path + "dg_" + std::to_string(m_frame_number) + ".png";
